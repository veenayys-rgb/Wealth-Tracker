"""Mom India Equity — NSE/BSE holdings. Display + Quick Edit + Add/Edit/Delete."""
...
st.set_page_config(page_title="Mom India Equity | Wealth Tracker", page_icon="📈", layout="wide")
st.title("📈 Mom — India Equity")
render_sidebar()
...
if holdings:
    ...
    st.info("No equity holdings for Mom yet. Add below.")
...
if holdings:
    with st.expander("✏️ Quick Edit"):
        ...
        edited = st.data_editor(
            ...
            hide_index=True, use_container_width=True, key="qe_mom_equity",
        )
        bc1, bc2 = st.columns(2)
        if bc1.button("💾 Save Changes", key="qsave_mom_equity"):
            price_rows = []
            ...
            st.rerun()
        if bc2.button("🗑️ Delete Selected", key="del_mom_equity"):
...
if holdings:
    with st.expander("✏️ Edit Holding"):
        options = [f"{h.get('symbol','')} — {h.get('company_name','')} ({h.get('holding_type','')})"
                   for h in holdings]
        sel = st.selectbox("Select holding", options, key="edit_mom_eq_sel")
        idx = options.index(sel)
        h   = holdings[idx]
        with st.form("edit_mom_equity"):
            c1, c2, c3 = st.columns(3)
            ...
            buy_date = c6.date_input("Buy Date", value=bd, key="edit_mom_bd")
            c7, c8 = st.columns(2)
