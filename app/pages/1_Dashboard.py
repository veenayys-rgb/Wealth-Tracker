"""
Dashboard — Full Net Worth Summary + Breakdown by Asset Class
"""
import sys, os
...
from utils.fmt    import inr, pct, metric_card
...
def intl_totals() -> tuple:
    prices = {r["symbol"]: (float(r["price"]), r["currency"])
              for r in fetch("equity_international_prices")}
    inv = cv = 0.0
...
c1.markdown(metric_card("Total Net Worth",  inr(total_cv)),  unsafe_allow_html=True)
c2.markdown(metric_card("Total Invested",   inr(total_inv)), unsafe_allow_html=True)
c3.markdown(metric_card("Gain / Loss",      inr(gain), delta=pct(ret_pct)), unsafe_allow_html=True)
...
st.caption("Insurance contribution = Surrender Value. Bank/FD/UAE in INR equivalent.")
st.caption("Charts coming in next phase.")
