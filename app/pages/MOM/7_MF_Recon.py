...
st.set_page_config(page_title="Mom MF Recon | Wealth Tracker", page_icon="🔍", layout="wide")
st.title("🔍 Mom — MF Reconciliation (CAMS)")
st.caption("Upload a CAMS Consolidated Account Summary PDF to compare with Mom's stored holdings.")
render_sidebar()
...
st.success(f"Parsed **{len(cams)} funds** for **{investor or 'Mom'}**.")

# ── Load stored ────────────────────────────────────────────────────────────────
stored        = load("mom_mutual_funds.json")
...
st.dataframe(
    view.style.format(fmt).map(_colour, subset=["Unit Diff", "NAV Diff", "Cost Diff (₹)"]),
...
if missing:
    ...
    st.warning(f"⚠️ {len(missing)} fund(s) in tracker not found in CAMS — review manually: {names}")
...
if st.button("🔄 Sync to Tracker", disabled=not can_sync, type="primary"):
    ...
    save("mom_mutual_funds.json", stored)
