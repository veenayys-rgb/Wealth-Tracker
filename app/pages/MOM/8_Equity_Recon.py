...
st.set_page_config(page_title="Mom Equity Recon | Wealth Tracker", page_icon="📋", layout="wide")
st.title("📋 Mom — Equity Reconciliation (HDFC Demat)")
st.caption("Upload an HDFC Depository Holding Statement PDF to compare with Mom's stored holdings.")
render_sidebar()
...
with st.expander("⚙️ DP Account → NRE/NRO Mapping (one-time setup)", expanded=not dp_map):
    st.caption(
        "Enter Mom's HDFC NRE and NRO DP Account numbers once — "
...
# ── UI ─────────────────────────────────────────────────────────────────────────
c1, c2 = st.columns(2)
uploaded = c1.file_uploader("HDFC PDF", type=["pdf", "PDF"])
...
if auto_ht:
    ...
    default_idx = 0
    if dp_account:
        c2.warning(f"DP Acct `{dp_account}` not in mapping — select manually or update config above.")
    else:
...
st.success(f"Parsed **{len(hdfc)} holdings** for **{investor or 'Mom'}** ({holding_type}).")

# ── Load stored + MF classification ───────────────────────────────────────────
stored         = load("mom_equity_india.json")
...
mf_stored = load("mom_mutual_funds.json")
...
if missing:
    ...
    st.warning(f"⚠️ {len(missing)} holding(s) in tracker not found in HDFC — review manually:\n\n{names}")

st.info("ℹ️ HDFC statement provides current price only — avg cost will not be updated. "
        "New stocks are added with avg cost = 0; update via the India Equity page.")
...
if st.button("🔄 Sync to Tracker", disabled=not can_sync, type="primary"):
    ...
    save("mom_equity_india.json", stored)
    ...
    if added:
        st.info("📝 New holdings added with avg cost = 0 and blank symbol. "
                "Please update via the India Equity page.")
    st.rerun()
