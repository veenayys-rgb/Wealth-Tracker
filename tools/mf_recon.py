...
Extraction logic adapted from mf_extractor.py (Desktop/Wealth_Tracker/MF Extractor)
"""
import sys, os, re, io
...
# ── Regex patterns (from mf_extractor.py) ─────────────────────────────────────
NOISE_RE = re.compile(
    r'(Page\s+\d+\s+of\s+\d+|'
    r'\d{4}-eviL|'
    r'\d+\.\d+V:noisreV|'
...
def extract_holdings(file_bytes: bytes, password: str) -> tuple[str, list[dict]]:
    ...
    for line in joined:
        ...
        raw_scheme  = (scheme_before + " " + scheme_after).strip()
        raw_scheme  = NOISE_RE.sub(" ", raw_scheme)
        raw_scheme  = re.sub(r'\s+', ' ', raw_scheme).strip()
        scheme_name = re.sub(r'^[^-]*-\s*', '', raw_scheme).strip()
